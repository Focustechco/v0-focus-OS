...
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(`${origin}/configuracoes?google=error&reason=no_code`)
  }
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(`${origin}/configuracoes?google=error&reason=db_error`)
    }
    ...
    return NextResponse.redirect(`${origin}/configuracoes?google=error&reason=${encodeURIComponent(err.message)}`)
  }
}
