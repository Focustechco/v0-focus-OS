    ...
    <Card className={`bg-[#141414] border-[#2A2A2A] ${delivery.status === "atrasado" ? "border-l-4 border-l-red-500" : delivery.status === "risco" ? "border-l-4 border-l-yellow-500" : ""} hover:border-orange-500/30 transition-colors`}>
      <CardContent className="p-4">
        ...
        <div className="p-3 bg-[#0A0A0A] rounded-lg border border-[#2A2A2A] mb-3">
          <div className="text-xs text-neutral-400 mb-1">Entrega:</div>
          <div className="text-sm text-white">{delivery.delivery}</div>
        </div>
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0A]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <FocusHeader title="PRAZOS & ENTREGAS" />
        ...
        <main className="flex-1 overflow-auto p-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-neutral-500">Timeline e deadlines dos projetos</p>
            </div>
          </div>
            ...
            <Card className="bg-[#141414] border-[#2A2A2A]">
              <CardContent className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-neutral-400">Entregues (mes)</div>
                </div>
          ...
          {/* Timeline Visual */}
          <Card className="bg-[#141414] border-[#2A2A2A] mb-6">
                ...
                {deliveries.filter(d => d.status !== "entregue").sort((a, b) => a.daysLeft - b.daysLeft).map((delivery) => {
                  ...
                  return (
                    <div key={delivery.id} className="flex items-center gap-4">
                      <div className="w-20 text-xs text-orange-500 font-mono flex-shrink-0">{delivery.id}</div>
                      <div className="flex-1 h-6 bg-[#0A0A0A] rounded relative overflow-hidden">
                        ...
                        />
                        <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] text-white font-medium truncate max-w-[200px]">
                          {delivery.name}
                        </span>
                      </div>
                      <div className={`w-24 text-right text-xs font-mono ${status.color}`}>
                        {delivery.daysLeft < 0 ? `${Math.abs(delivery.daysLeft)}d atrasado` : `${delivery.daysLeft}d`}
                      </div>
                      <div className="w-20 text-xs text-neutral-500 font-mono">{delivery.deadline}</div>
                    </div>
                  )
                })}
              </div>
