    ...
    <Card className={`bg-[#141414] border-[#2A2A2A] ${approval.priority === "urgente" ? "border-l-4 border-l-red-500" : ""} hover:border-orange-500/30 transition-colors`}>
      <CardContent className="p-3 sm:p-4">
        <div className="flex items-start justify-between mb-3 gap-2">
          ...
          <Badge variant="outline" className={`text-[9px] ${priority.bg} ${priority.color} flex-shrink-0`}>
            {priority.label}
          </Badge>
        </div>

        <h3 className="text-sm font-medium text-white mb-1">{approval.title}</h3>
        <p className="text-[10px] text-neutral-500 mb-3 line-clamp-2">{approval.description}</p>

        <div className="flex items-center gap-2 sm:gap-4 text-[10px] text-neutral-400 mb-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-[#2A2A2A] flex-wrap">
                ...
                <div className="p-4 bg-[#0A0A0A] rounded-lg border border-[#2A2A2A] space-y-2">
                  {Object.entries(approval.details).map(([key, value]) => (
                    <div key={key} className="flex justify-between text-sm gap-2">
                      <span className="text-neutral-500 capitalize flex-shrink-0">{key.replace(/([A-Z])/g, ' $1').trim()}:</span>
          ...
          <Button size="sm" variant="outline" className="h-8 text-xs border-red-500/50 text-red-500 hover:bg-red-500/10 px-2">
            <XCircle className="w-3 h-3" />
          </Button>
          <Button size="sm" className="h-8 text-xs bg-green-500 hover:bg-green-600 px-3">
    ...
    <div>
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-lg sm:text-xl font-display font-bold text-white">Aprovacoes</h1>
            <p className="text-xs sm:text-sm text-neutral-500">Fila de aprovacoes pendentes</p>
          </div>
          <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-red-500/10 border border-red-500/30 rounded-lg">
              <AlertTriangle className="w-3 h-3 sm:w-4 sm:h-4 text-red-500" />
              <span className="text-xs sm:text-sm text-red-500 font-mono">{urgentCount}</span>
            </div>
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              <Clock className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-500" />
              <span className="text-xs sm:text-sm text-yellow-500 font-mono">{normalCount}</span>
            </div>
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-neutral-500/10 border border-neutral-500/30 rounded-lg">
              <Clock className="w-3 h-3 sm:w-4 sm:h-4 text-neutral-400" />
              <span className="text-xs sm:text-sm text-neutral-400 font-mono">{waitingCount}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
        <Card className="bg-[#141414] border-[#2A2A2A]">
          <CardContent className="p-3 sm:p-4 flex items-center justify-between">
            <div>
              <div className="text-xl sm:text-2xl font-bold text-white font-mono">{approvals.length}</div>
              <div className="text-[10px] sm:text-xs text-neutral-500">Total Pendentes</div>
            </div>
        ...
        <Card className="bg-[#141414] border-[#2A2A2A]">
          <CardContent className="p-3 sm:p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-[10px] sm:text-xs text-neutral-500">Aguard. Gabriel</div>
            </div>
        ...
        <Card className="bg-[#141414] border-[#2A2A2A]">
          <CardContent className="p-3 sm:p-4 flex items-center justify-between">
            <div>
              <div className="text-xl sm:text-2xl font-bold text-white font-mono">1</div>
              <div className="text-[10px] sm:text-xs text-neutral-500">Aguard. Cliente</div>
            </div>
        ...
        <Card className="bg-[#141414] border-[#2A2A2A]">
          <CardContent className="p-3 sm:p-4 flex items-center justify-between">
            <div>
              <div className="text-xl sm:text-2xl font-bold text-red-500 font-mono">{urgentCount}</div>
              <div className="text-[10px] sm:text-xs text-neutral-500">Prioridade ALTA</div>
            </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xs sm:text-sm font-medium text-neutral-400 tracking-wider flex items-center gap-2">
        ...
        <h2 className="text-xs sm:text-sm font-medium text-neutral-400 tracking-wider flex items-center gap-2 pt-4">
