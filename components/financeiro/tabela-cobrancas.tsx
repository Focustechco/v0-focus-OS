      ...
      <table className="w-full border-collapse text-[11px]">
        <thead>
          <tr className="border-b border-border bg-secondary/50">
            <th className="text-left p-3 sm:p-4 text-neutral-500 font-mono font-medium tracking-wider text-[10px]">ID</th>
            <th className="text-left p-3 sm:p-4 text-neutral-500 font-mono font-medium tracking-wider text-[10px]">CLIENTE</th>
            <th className="text-left p-3 sm:p-4 text-neutral-500 font-mono font-medium tracking-wider text-[10px]">TIPO</th>
            <th className="text-right p-3 sm:p-4 text-neutral-500 font-mono font-medium tracking-wider text-[10px]">VALOR</th>
            <th className="text-left p-3 sm:p-4 text-neutral-500 font-mono font-medium tracking-wider text-[10px]">VENCIMENTO</th>
            <th className="text-left p-3 sm:p-4 text-neutral-500 font-mono font-medium tracking-wider text-[10px]">STATUS</th>
            <th className="text-right p-3 sm:p-4 text-neutral-500 font-mono font-medium tracking-wider text-[10px]">AÇÕES</th>
          </tr>
        </thead>
            ...
            <tr>
              <td colSpan={7} className="p-12 text-center text-neutral-600">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-[10px] text-neutral-700">Crie uma nova cobrança para começar.</span>
                </div>
              </td>
            </tr>
          ) : (
            cobrancas.map((cob: any) => {
              ...
              return (
                <tr key={cob.id} className="hover:bg-accent/5 transition-colors group">
                  <td className="p-3 sm:p-4 font-mono text-orange-500/70 text-[10px]">
                    {cob.id.substring(4, 12)}
                  </td>
                  <td className="p-3 sm:p-4 text-foreground font-medium text-xs">
                    {cob.customerName || cob.customer?.substring(0, 12)}
                  </td>
                  <td className="p-3 sm:p-4 text-neutral-400 text-[10px]">
                    {BILLING_TYPE_MAP[cob.billingType] || cob.billingType}
                  </td>
                  <td className="p-3 sm:p-4 font-mono text-foreground text-right font-bold">
                    {formatBRL(cob.value)}
                  </td>
                  <td className={`p-3 sm:p-4 font-mono text-[10px] ${isOverdue ? 'text-red-500' : 'text-neutral-400'}`}>
